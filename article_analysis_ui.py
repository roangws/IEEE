...
def render_article_analysis():
    ...
    st.header("📊 Article Analysis - IEEE Access Submission Evaluation")
    st.markdown("""
    ...
    if not uploaded_file:
        st.info("👆 Upload a manuscript file to begin analysis")
        
        # Show example metrics
        st.divider()
        st.subheader("📈 IEEE Access Benchmarks (5,634 papers)")
        ...
        st.divider()
        st.subheader("📚 IEEE Access Journal Information")
        ...
        st.divider()
        st.subheader("🎯 Acceptance Criteria & Desk Rejection Triggers")
        
        col1, col2 = st.columns(2)
        
        with col1:
            st.markdown("""
            **✅ Acceptance Threshold Indicators:**
        ...
        st.divider()
        st.subheader("📊 Statistical Benchmarks from 5,634 Published Papers")
        ...
        st.divider()
        st.subheader("🔬 Common Research Topics & Keywords")
        ...
        st.divider()
        st.subheader("📈 Citation Patterns & Reference Quality")
        ...
        **Citation Recency Benchmarks:**
        ...
        💡 **Tip**: Bias toward recent work (post-2017) but include foundational citations where appropriate.
        """)
        return
    
    # Process uploaded file
    st.divider()
    st.subheader("🔄 Processing Manuscript")
    ...
    try:
        ...
        with summary_col2:
            ...
            section_data = [
                ['Section', 'Present'],
                ['Abstract', '✓' if metrics.has_abstract else '✗'],
                ['Introduction', '✓' if metrics.has_introduction else '✗'],
                ['Methodology', '✓' if metrics.has_methodology else '✗'],
                ['Experiments', '✓' if metrics.has_experiments else '✗'],
                ['Results', '✓' if metrics.has_results else '✗'],
                ['Discussion', '✓' if metrics.has_discussion else '✗'],
                ['Conclusion', '✓' if metrics.has_conclusion else '✗'],
                ['References', '✓' if metrics.has_references else '✗']
            ]
        ...
        # Metrics comparison
        st.divider()
        st.subheader("📈 Metrics vs IEEE Access Benchmarks")
        ...
        if evaluation.desk_rejection_reasons:
            st.divider()
            st.error("🚫 Desk Rejection Risk Factors")
            for reason in evaluation.desk_rejection_reasons:
                st.markdown(f"- {reason}")
        
        # Extracted metrics (expandable)
        st.divider()
        with st.expander("🔍 Detailed Extracted Metrics"):
            st.json({
                "word_count": metrics.word_count,
                "abstract_length": metrics.abstract_length,
        ...
        st.error(f"❌ Error analyzing manuscript: {str(e)}")
        import traceback
