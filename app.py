...
# Enable debug mode via environment variable (will be used by Streamlit)
os.environ["STREAMLIT_LOGGER_LEVEL"] = "debug"

# Try to import HTML-to-PDF generator with MathJax (best option - perfect formulas)
try:
    # Note: HTMLMathJaxPDFGenerator was replaced by MathJaxPDFGenerator
    # Keeping the try/except for compatibility but using the new generator
    MATHJAX_PDF_AVAILABLE = True
    print("✅ MathJax PDF generator available")
except Exception as e:
    MATHJAX_PDF_AVAILABLE = False
    print(f"⚠️  MathJax PDF generator not available: {e}")
...
def _save_ui_cache(state: dict):
    try:
        ...
        backup_path = os.path.join(_UI_CACHE_DIR, f"ui_cache_backup_{timestamp}.json")
        ...
        print(f"Warning: Failed to save UI cache: {e}")
...
def _validate_and_fix_latex_delimiters(article: str, log_fn=None) -> str:
    ...
    if errors_found:
        _log(f"⚠️ Detected {len(errors_found)} LaTeX syntax errors: {', '.join(errors_found[:5])}")
        _log("🔧 Applying comprehensive auto-fix...")
        ...
        if remaining_errors:
            _log(f"❌ Auto-fix incomplete - {len(remaining_errors)} errors remain: {', '.join(remaining_errors)}")
            _log("⚠️ Manual review required - some formulas may not render correctly")
        else:
            _log("✅ Auto-fix successful - all LaTeX syntax errors corrected")
            changes = sum(1 for a, b in zip(original, article) if a != b)
            _log(f"📝 Applied {changes} character corrections")
    else:
        _log("✅ No LaTeX syntax errors detected")
...
def generate_article_with_openai_chunks(
    ...
    locally into a compact “evidence bank”, then generate the article in multiple
    smaller OpenAI calls with pacing/retry.
    """

    # tiktoken is already imported at module level
    ...
    def _call_openai_paced(prompt_text: str, *, max_tokens: int, system: str, tpm_limit: int = 30000):
        ...
        if used + req_tokens > safety_limit:
            ...
            _log(f"⏳ OpenAI TPM pacing: waiting {wait_s}s to stay under rate limits...")
            time.sleep(wait_s)
        ...
        for attempt in range(3):
            try:
                ...
                if "429" in msg or "rate_limit" in msg or "TPM" in msg:
                    wait_s = 65
                    _log(f"⚠️ OpenAI rate-limited. Waiting {wait_s}s and retrying (attempt {attempt + 1}/3)...")
                    time.sleep(wait_s)
                    window_start = time.time()
                    used = 0
                    continue
...
"""
        ...
        _log(f"OpenAI section '{section_name}': input≈{in_tokens:,} tokens, output_max={max_out_tokens}")
        ...
        _log(f"OpenAI section '{section_name}': output≈{out_tokens:,} tokens, time={dt:.1f}s, est_cost=${total_cost_usd:.4f}")
        outputs.append(part)

    total_time = time.perf_counter() - start_time
    _log(f"OpenAI total: input≈{total_in_tokens:,} tokens, output≈{total_out_tokens:,} tokens, time={total_time:.1f}s, est_cost=${total_cost_usd:.4f}")

    article = "\n\n".join(outputs)
    
    # POST-GENERATION VALIDATION AND AUTO-FIX
    _log("🔍 Validating LaTeX syntax...")
...
<script>
// Enable debug mode
console.log("🔍 Debug mode enabled for LaTeX rendering");
...
if (typeof katex !== 'undefined') {
    ...
    katex.ParseError = function(message, lexer, position) {
        console.error("🚨 KaTeX Parse Error:", message, "at position", position);
        console.error("📝 Context:", lexer.input.substr(Math.max(0, position-50), 100));
    ...
    katex.render = function(expression, element, options) {
        console.log("🧮 Rendering LaTeX:", expression);
        try {
    ...
    console.log("🔧 KaTeX debug hooks installed");
} else {
    console.warn("⚠️ KaTeX not found - math rendering may not work");
}
        ...
        mutation.addedNodes.forEach(function(node) {
            if (node.nodeType === 1) {
                if (node.classList && node.classList.contains('katex')) {
                    console.log("📐 KaTeX element detected:", node);
                }
                if (node.classList && node.classList.contains('katex-error')) {
                    console.error("🚨 KaTeX error element detected:", node);
                }
            }
        });
    });
...
console.log("👁️ DOM observer for math elements installed");
</script>
...
def generate_ieee_refined_pdf(article_text: str, fallback_title: str) -> bytes:
    """
    Generate IEEE-style PDF for refined article.
    Uses MathJax PDF generation if available (perfect formulas),
    falls back to ReportLab if not.
    """
    # Try MathJax-based PDF generation first (if available)
    try:
        from html_mathjax_pdf_generator import HTMLMathJaxPDFGenerator
        generator = HTMLMathJaxPDFGenerator()
        
        # Normalize article for rendering
        normalized_article = _normalize_article_for_rendering(article_text)
        
        # Extract title from normalized article
        citation_manager = CitationManager()
        extracted_title, _ = citation_manager.extract_title(normalized_article)
        final_title = extracted_title if extracted_title != "Research Article" else fallback_title
        
        # Generate PDF with MathJax rendering
        print("Generating HTML MathJax PDF for refined article: " + final_title)
        pdf_buffer = generator.generate_pdf(normalized_article, final_title)
        print("HTML MathJax PDF generated successfully for refined article")
        return pdf_buffer
    except Exception as e:
        # Fall back to ReportLab if MathJax PDF fails
        print(f"HTML MathJax PDF generation failed for refined article, using ReportLab: {e}")
        import traceback
        traceback.print_exc()
        pass
...
def _clean_abstract_formulas_and_citations(article_text: str) -> str:
    """
    Post-process function to remove formulas, equations, and citations from the abstract.
    This ensures compliance with IEEE abstract requirements even if LLM doesn't follow instructions.
    """
    import re
    ...
    for line in lines:
        ...
        if in_abstract and not abstract_end:
            # Remove citations like [1], [2-5], [12, 15]
            cleaned = re.sub(r'\[\d+(?:,\s*\d+)*(?:-\d+)?\]', '', line)
            ...
            # Remove \begin{equation}...\end{equation} and similar
            cleaned = re.sub(r'\\begin\{(?:equation|align|gather|multline)\*?\}.*?\\end\{(?:equation|align|gather|multline)\*?\}', '', cleaned, flags=re.DOTALL)
            
            # Clean up extra spaces and punctuation
            cleaned = re.sub(r'\s+', ' ', cleaned)  # Multiple spaces to single
            cleaned = re.sub(r'\s*([.,;])\s*', r'\1 ', cleaned)  # Clean up punctuation spacing
            cleaned = re.sub(r'\s*\n\s*', '\n', cleaned)  # Clean line endings
            cleaned = cleaned.strip()
            
            if cleaned:  # Only add non-empty lines
                abstract_lines.append(cleaned)
        else:
            # Non-abstract content, add as-is
            abstract_lines.append(line)
    
    return '\n'.join(abstract_lines)


def _fix_broken_tables(article_text: str) -> str:
    """
    Post-process function to fix broken tables with LaTeX math expressions.
    Converts LaTeX math in tables to plain text representation.
    Only affects tables, preserves LaTeX in other sections.
    """
    import re
    ...
    for line in lines:
        ...
        if '|' in line and ('|' in line.split('|')[1] if len(line.split('|')) > 1 else False):
            ...
            # Replace <span class="katex">...</span> with the plain text version
            line = re.sub(r'<span class="katex"[^>]*>.*?<annotation encoding="application/x-tex">([^<]+)</annotation>.*?</span>', r'\1', line, flags=re.DOTALL)
            
            # Replace remaining HTML tags
            line = re.sub(r'<[^>]+>', '', line)
            
            # Replace Greek letters with names (only in tables)
            greek_replacements = {
                'α': 'alpha',
                'β': 'beta', 
                'γ': 'gamma',
            ...
            # Line without pipe likely ends table
            in_table = False
...
def generate_article_pdf(article_text, topic, citation_map, sources):
    ...
    # Try MathJax-based PDF generation first (if available)
    try:
        from mathjax_pdf_generator import MathJaxPDFGenerator
        generator = MathJaxPDFGenerator()
...
def init_session_state():
    ...
    if 'active_collection' not in st.session_state:
        st.session_state.active_collection = 'academic_papers_100_specter'
    ...
    if 'refine_instructions' not in st.session_state:
        st.session_state.refine_instructions = None
    ...
    if not st.session_state._ui_cache_loaded:
        ...
        if isinstance(cached, dict):
            ...
            if st.session_state.answer_result is None and cached.get('answer_result') is not None:
                ...
                cache_restored = True
                items_restored.append("Q&A results")
            ...
            if st.session_state.generated_article is None and cached.get('generated_article') is not None:
                ...
                cache_restored = True
                items_restored.append("Generated article")
                if cached.get('refined_article'):
                    items_restored.append("Refined article")
                ...
                if cached.get('word_count') is not None:
                    st.session_state.word_count = cached.get('word_count')
                if cached.get('selected_model') is not None:
                    st.session_state.selected_model = cached.get('selected_model')
                if cached.get('selected_llm') is not None:
                    st.session_state.selected_llm = cached.get('selected_llm')
...
def render_header():
    ...
    st.title("📚 Academic Paper Analysis & Generation")
    st.markdown("""
...
def render_analysis_section():
    ...
    st.header("🔍 Section 1: Paper Analysis & Q&A")
    ...
    with st.expander("⚙️ Advanced Options"):
        # API Keys section
        st.markdown("**API Keys** - Configure your API keys")
        
        # Show status of loaded keys
        openai_key_loaded = os.getenv('OPENAI_API_KEY', '')
        if openai_key_loaded:
            st.info(f"✅ OpenAI key loaded from .env (starts with: {openai_key_loaded[:10]}...)")
        else:
            st.warning("⚠️ No OpenAI key found in .env file")
        
        st.markdown("*You can override keys here if needed.*")
        ...
        with col_key2:
            ...
            if anthropic_key != os.getenv('ANTHROPIC_API_KEY', ''):
                ...
                st.success("✅ Anthropic key updated")
    ...
    if st.button("🔍 Search", type="primary", width='stretch'):
        if not query:
            ...
            st.write(f"Debug: Using collection '{collection}'")  # Debug output
    ...
    if st.session_state.answer_result:
        ...
        if citations:
            st.success(f"✅ Found {len(citations)} citation(s) in the answer")
        else:
            st.warning("⚠️ No citations found in the answer. The response may not be based on the provided sources.")
        ...
        for i, source in enumerate(result['sources'], 1):
            ...
            with st.expander(f"📄 [{i}] {title} (Relevance: {source['score']:.2%})"):
...
def render_research_analysis():
    ...
    st.header("🔬 Research Landscape Analysis")
    ...
    try:
        ...
        st.success(f"✓ Using collection: **{collection}** ({collection_info.points_count:,} vectors)")
    except Exception as e:
    ...
    # Run Analysis button
    if st.button("🔬 Run Research Analysis", type="primary", width='stretch'):
        with st.spinner(f"Analyzing up to {max_papers} papers... (this may take 30-60 seconds)"):
            try:
                ...
                if custom_focus and custom_focus.strip():
                    ...
                    st.info(f"🎯 Focusing analysis on: {', '.join(focus_keywords)}")
    ...
    if 'analysis_results' in st.session_state:
        ...
        st.subheader("🎯 Research Themes Detected")
        ...
        st.info("👆 Click 'Run Research Analysis' to discover insights from your paper collection")
...
def render_paper_explorer():
    ...
    st.header("📚 Paper Explorer & Similar Papers")
    ...
    # Search interface
    search_query = st.text_input("🔍 Search for papers by topic or keywords", 
                                placeholder="e.g., deep learning for medical imaging")
    
    num_results = st.slider("Number of results", 5, 50, 10)
    
    if st.button("🔎 Search Papers", type="primary") and search_query:
    ...
    if paper_options:
        ...
        if selected_paper and st.button("🔍 Find Similar Papers"):
    ...
    if 'similar_papers' in st.session_state and st.session_state.similar_papers:
        st.divider()
        st.subheader("🔗 Similar Papers Network")
...
def render_synthesis_section():
    ...
    # Template editor
    with st.expander("📝 Template Editor (Optional - Customize Article Structure)"):
    ...
    with col1:
        # Initialize word_count in session state if not set
        if 'word_count' not in st.session_state or st.session_state.word_count is None:
            st.session_state.word_count = 4800  # Default word count
            # Debug: Show we're setting default
            st.write("Debug: Setting default word count to 4800")
        else:
            # Debug: Show current value
            st.write(f"Debug: word_count from session state: {st.session_state.word_count}")
        ...
        st.markdown(f"<small>Selected: **{word_count}** words</small>", unsafe_allow_html=True)
        
        # Save the word count to cache whenever it changes
        if word_count != st.session_state.get('last_saved_word_count'):
            _save_ui_cache({'word_count': word_count})
            st.session_state.last_saved_word_count = word_count
    ...
    if include_math:
        st.info("📐 Will include: equations, formulas, algorithmic complexity, mathematical models\n\n" +
                "✓ Aligns with 99% of IEEE papers (avg 41 math indicators)")
    
    if include_synthetic_data:
        st.info("📊 Will include: synthetic datasets, sample data tables, statistical analysis examples\n\n" +
                "✓ 59% of papers report error bars, 33% use std, 19% use ± symbols")
    
    # Add quality metrics recommendations
    with st.expander("📊 IEEE Paper Quality Benchmarks (from 5,634 papers)"):
    ...
    with st.expander("📋 Live Generation Log", expanded=True):
        log_area = st.empty()
    ...
    if st.button("🚀 Generate Article", type="primary", width='stretch'):
        if not topic:
                ...
                try:
                    ...
                    if os.path.exists(metadata_path):
                        ...
                        if potentially_irrelevant and len(potentially_irrelevant) > len(sources_list) * 0.3:
                            st.warning(f"⚠️ {len(potentially_irrelevant)} papers may not match your topic. Consider refining your search query for better relevance.")
                    ...
                    if actual_papers < num_papers:
                        st.warning(f"⚠️ Only found {actual_papers} unique papers for this topic (requested {num_papers}). Consider broadening your topic or reducing paper count.")
                    else:
                        st.success(f"✓ Retrieved {len(sources_list)} chunks from {actual_papers} unique papers. The LLM will cite these throughout the article.")
                    ...
                    try:
                        ...
                        status_text.text("📝 Preparing generation prompt...")
                        ...
                        if include_math:
                            add_log("📐 Mathematical functions: ENABLED")
                        if include_synthetic_data:
                            add_log("📊 Synthetic data generation: ENABLED")
                        
                        # Step 2: Send to LLM
                        progress_bar.progress(20)
                        status_text.text(f"🤖 Sending request to {llm_choice}...")
...
6. Articles without citations are UNACCEPTABLE"""
                        ...
                        if llm_choice == "Ollama Qwen 3 (Local, Free)":
                            ...
                            add_log("🔍 Validating LaTeX syntax...")
...
"""
                        ...
                        try:
                            ...
                            if body_word_count < min_target:
                                add_log(f"⚠️ Body word count ({body_word_count}) is below target minimum ({min_target})")
                                ...
                                add_log(f"✓ Body word count: {body_word_count} (target: {word_count})")
                        except Exception:
                            st.session_state._article_under_target = None
                        ...
                        st.success(f"✓ Article generated successfully! Used {citation_stats['citations_in_article']} citations from {citation_stats['cited_papers']} papers.")
                        ...
                        if st.session_state.get('_article_under_target'):
                            ...
                            st.warning(
                                f"⚠️ **Article Under Target Word Count** — Body has {under_info['body_words']} words "
                        ...
                        if ieee_compliant:
                            st.success("✅ **IEEE Standards Met** - Article meets all minimum requirements")
                        else:
                            st.warning("⚠️ **IEEE Standards Not Met** - Article may be rejected for export")
                        ...
                        with col2:
                            ref_status = "✅" if citation_stats.get('meets_min_references', False) else "❌"
                            st.metric(
                        ...
                        with col3:
                            cite_status = "✅" if citation_stats.get('meets_min_citations', False) else "❌"
                            st.metric(
                        ...
                        with col4:
                            density_status = "✅" if citation_stats.get('meets_min_density', False) else "❌"
                            st.metric(
                        ...
                        if citation_stats.get('has_orphaned', False):
                            ...
                            st.error(f"🔴 CRITICAL: {orphaned_count} citations lack reference mappings. Article cannot be exported until fixed.")
                            st.caption(f"Orphaned citations: {orphaned_list[:20]}")
                            st.caption(f"Valid citation range: [1] to [{max_valid}] ({len(citation_map)} papers)")
                            st.warning("⚠️ The LLM used citation numbers beyond available sources. Try regenerating with clearer instructions.")
                        ...
                        if papers_cited < papers_provided:
                            ...
                            st.info(f"ℹ️ **Citation Utilization:** The LLM cited {papers_cited} out of {papers_provided} papers provided ({utilization:.0f}%). "
                        ...
                        if not citation_stats.get('meets_min_references', False):
                            ...
                            st.warning(f"⚠️ Add {needed} more cited references to meet IEEE minimum (currently {citation_stats['cited_papers']}, need 19)")
                        if not citation_stats.get('meets_min_citations', False):
                            st.warning(f"⚠️ Add {68 - citation_stats['citations_in_article']} more in-text citations to meet IEEE minimum")
                        if not citation_stats.get('meets_min_density', False):
                            st.warning("⚠️ Citation density below IEEE minimum (3.5 refs/1k words)")
    ...
    if st.session_state.get('generated_article'):
        ...
        with col2:
            ...
            if st.session_state.generated_article:
                if st.button("🔧 Validate & Fix LaTeX", help="Check and fix LaTeX formula syntax errors"):
                    with st.spinner("Validating and fixing LaTeX syntax..."):
                        ...
                        if changes > 0:
                            st.success(f"✅ Fixed LaTeX syntax! Applied {changes} character corrections.")
                            st.info("💾 Changes saved. Reloading page...")
                        else:
                            st.success("✅ Article validated - no changes needed.")
        ...
        with col3:
            if st.session_state.generated_article:
                ...
                if missing:
                    ...
                    st.warning(f"⚠️ {len(missing)} orphaned citations will show as 'MISSING REFERENCE' in PDF")
        ...
        if article_text:  # Safety check
            ...
            if errors_before or errors_after:
                ...
                if errors_before:
                    st.error("⚠️ Click the '🔧 Validate & Fix LaTeX' button to fix the source article.")
                
                # If errors persist after normalization, that's a bug
                if errors_after:
                    st.error("❌ Normalization failed to fix errors - this is a bug. Please report.")
        ...
        if 'citation_map' in st.session_state and 'article_sources' in st.session_state and st.session_state.generated_article:
            with st.expander("📊 View Citation Analysis Report"):
                citation_manager = CitationManager()
        ...
        if st.session_state.get('generated_article') and 'citation_map' in st.session_state:
            ...
            from layer2_step29_integration import render_step_2_9_integration
            
            # Layer 2A: Fetch external references
            render_layer2a_fetch_external_refs()
            
            # Layer 2.9: Integrate external references (only if fetched)
            if st.session_state.get('external_references'):
                render_step_2_9_integration()
        ...
        # This step is always available if you have a generated article - external refs are optional
        if st.session_state.get('generated_article') and 'citation_map' in st.session_state:
            st.divider()
            st.markdown(
                """
...
""",
            ...
            st.markdown('<a id="step4"></a><div class="step_banner step_5_banner">Step 5 — Advanced: Local Corpus Refinement</div>', unsafe_allow_html=True)
            ...
            # Use article_sources if available, otherwise use empty list (allows refinement without local corpus)
            ...
            # Build section-specific expansion/reduction recommendations from analysis
            # Use the SAME data that was displayed in the UI above
            needs_change = []  # Sections that need either expansion or reduction
            for s in section_analysis:
                gap = s['Gap']
                # Include sections that need significant changes (>10 words difference)
                if abs(gap) > 10:
                    needs_change.append(s)
                # Always include abstract if it needs reduction (regardless of threshold)
                elif s['Section'] == 'Abstract' and gap < 0:
                    needs_change.append(s)
                # Debug: Ensure abstract is always included if it needs change
                if s['Section'] == 'Abstract' and abs(gap) > 0:
                    # Check if it's already in needs_change
                    if not any(item['Section'] == 'Abstract' for item in needs_change):
                        needs_change.append(s)
            
            if needs_change:
                # Sort by absolute gap size (largest difference first)
                needs_change.sort(key=lambda x: abs(x['Gap']), reverse=True)
                
                # Ensure abstract is at the top if it needs reduction
                abstract_idx = next((i for i, s in enumerate(needs_change) if s['Section'] == 'Abstract'), None)
                if abstract_idx is not None and abstract_idx > 0:
                    # Move abstract to the front
                    abstract_item = needs_change.pop(abstract_idx)
                    needs_change.insert(0, abstract_item)
                
                section_expansion_guidance = "\n\n**SECTION-SPECIFIC WORD COUNT ADJUSTMENTS:**\n"
                section_expansion_guidance += "Based on IEEE standards analysis, adjust these sections in priority order:\n\n"
                for i, s in enumerate(needs_change[:5], 1):
                    gap = s['Gap']
                    
                    # Determine action needed
                    if gap > 0:
                        action = "EXPAND"
                        status_text = f"Need +{gap} words"
                    else:
                        action = "REDUCE"
                        status_text = f"Need {gap} words"
                    
                    section_expansion_guidance += f"{i}. **{s['Section']}** - {action}\n"
                    section_expansion_guidance += f"   - Current: {s['Current']} words\n"
                    section_expansion_guidance += f"   - Target: {s['Target']} words\n"
                    section_expansion_guidance += f"   - Action needed: {status_text}\n"
                    section_expansion_guidance += f"   - Progress: {s['% of Target']}\n"
                    
                    # Add specific instructions for abstract
                    if s['Section'] == 'Abstract' and action == "REDUCE":
                        section_expansion_guidance += "   - SPECIAL: Remove ALL mathematical formulas, equations, citations, and references\n"
                        section_expansion_guidance += "   - SPECIAL: Keep only essential technical contributions, methods, key findings, conclusions\n"
                    section_expansion_guidance += "\n"
                
                section_expansion_guidance += "**Instructions:**\n"
                section_expansion_guidance += "- For EXPANSION: Add detailed explanations, examples, analysis\n"
                section_expansion_guidance += "- For REDUCTION: Remove redundancy, tighten sentences, be concise\n"
                section_expansion_guidance += "- Keep technical accuracy and key content intact\n"
                section_expansion_guidance += "- Maintain proper paragraph structure and flow\n\n"
            
            # Check if word count adjustment is needed
            word_count_diff = target_word_count - current_word_count
            if abs(word_count_diff) > 50:  # Only show if difference is significant
                if word_count_diff > 0:
                    word_expansion_note = f"\n\n**WORD COUNT EXPANSION REQUIRED:**\n- Current: {current_word_count} words\n- Target: {target_word_count} words\n- Add approximately {word_count_diff} words total\n"
                else:
                    word_expansion_note = f"\n\n**WORD COUNT REDUCTION REQUIRED:**\n- Current: {current_word_count} words\n- Target: {target_word_count} words\n- Reduce approximately {abs(word_count_diff)} words total\n- Focus on removing redundancy while keeping key content\n"
                    
                    # Add specific abstract reduction guidance if abstract is over target
                    abstract_section = next((s for s in section_analysis if 'Abstract' in s['Section']), None)
                    if abstract_section and abstract_section['Gap'] < 0:
                        word_expansion_note += f"\n\n**ABSTRACT REDUCTION PRIORITY:**\n- Abstract: {abstract_section['Current']} words (target: {abstract_section['Target']})\n- Remove redundant phrases, combine sentences, eliminate filler words\n- CRITICAL: Remove ALL mathematical formulas, equations, citations, and references\n- Keep only essential technical contributions, methods, key findings, conclusions\n"
...
- ALL existing in-text citations (e.g., [1], [2-5], [12,15]) MUST be preserved exactly as they appear
- DO NOT remove, modify, or renumber any existing citations
- DO NOT delete any citations from the article body
- Citation numbers MUST remain consistent with the reference list
- If adding content, you may add NEW citations to existing references
- Never remove citations to reduce word count - remove other text instead

Focus on:
- Improving clarity and flow without changing meaning
- Fixing grammar, consistency, and IEEE tone
- Removing repetition and tightening sentences where needed (but keep citations)
- Ensuring terminology and notation are consistent
- Adjusting word count to meet targets (expand or reduce as specified)


CRITICAL ABSTRACT REQUIREMENTS:
- The Abstract MUST NOT contain any mathematical formulas, equations, or symbols
- The Abstract MUST NOT contain any citations or references (e.g., [1], (Smith, 2023))
- The Abstract MUST NOT contain LaTeX expressions like $...$, \[...\], \begin{equation}...\end{equation}
- The Abstract MUST NOT contain mathematical notation like softmax, fractions, square roots, etc.
- IMPORTANT: When reducing the abstract, you MUST DELETE all formulas, equations, and citations
- Replace mathematical concepts with plain English descriptions only
- Fix ALL paragraph formatting - ensure proper paragraph breaks (empty line between paragraphs)
- Each paragraph should be 3-5 sentences maximum, not one giant block of text
- Use proper transitions between paragraphs
- Fix any broken tables - ensure tables use proper Markdown format with | separators
- Replace LaTeX math in tables with plain text (e.g., α -> alpha, β -> beta)
- Keep the abstract concise and focused on essential contributions only

Do NOT add a References section.
Do NOT add new citations unless explicitly instructed.""" + (reference_preservation_note or "") + (section_expansion_guidance or "") + (word_expansion_note or "")

            # Initialize refine_instructions in session state if not set or empty
            if not st.session_state.get('refine_instructions'):
                st.session_state.refine_instructions = default_refine_instructions
            ...
            with st.expander("🔍 Debug: Section Analysis Source of Truth", expanded=False):
                    ...
                    try:
                        ...
                        if st.session_state.get('enable_web_search'):
                            ...
                            try:
                                ...
                                add_refine_log(f"✅ Found {len(external_refs_for_refiner)} external papers")
                                ...
                                add_refine_log(f"⚠️ Web search failed: {str(e)}")
                        ...
                        # Post-process to ensure abstract compliance
                        add_refine_log("Post-processing abstract to remove formulas and citations...")
                        refined_article = _clean_abstract_formulas_and_citations(refined_article)
                        add_refine_log("✅ Abstract cleaned successfully")
                        
                        # Fix broken tables with LaTeX expressions
                        add_refine_log("Fixing broken tables and LaTeX expressions...")
                        refined_article = _fix_broken_tables(refined_article)
                        add_refine_log("✅ Tables fixed successfully")
                        ...
                        if st.session_state.get('unified_reference_list'):
                            ...
                            if missing_citations:
                                add_refine_log(f"⚠️ Warning: {len(missing_citations)} references not cited in refined article: {sorted(list(missing_citations))[:10]}")
                            ...
                            add_refine_log(f"✅ Preserved all {len(all_unified_refs)} references from Step 4")
                        else:
                        ...
                        if external_refs_for_refiner:
                            add_refine_log(f"✅ Added {len(external_refs_for_refiner)} external references to References section")

                        progress_bar.progress(100)
                        status_text.text("✅ Layer 2 refinement complete")
                        st.success(f"✅ Layer 2 complete. Added {report.citations_added} citations.")
                        ...
                        add_refine_log("💾 Saved Step 5 results to cache")

                    except Exception as e:
                        st.session_state.last_refinement_error = str(e)
                        progress_bar.progress(100)
                        status_text.text("❌ Refinement failed")
            ...
            if st.session_state.get('refinement_report'):
                ...
                with col3:
                    st.metric("Validation", "✅ Passed" if report.validation_passed else "⚠️ Issues")
            ...
            if st.session_state.get('refined_article'):
                st.divider()
                st.subheader("✅ Layer 2 Output: Refined Article")
                cm = CitationManager()
                ...
                with col_a:
                    if st.button("✅ Use Refined Version as Active Article", width='stretch', key="promote_refined"):
                ...
                with col_c:
                    ...
                    if st.session_state.get('refined_article_pdf_bytes') is None or st.session_state.get('refined_article_pdf_sig') != refined_sig:
                        # Use the refined article with all post-processing applied
                        article_for_pdf = st.session_state.refined_article
                        try:
